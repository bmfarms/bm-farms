  ...
  return (
    <section className="py-16 bg-gradient-to-br from-emerald-50/60 via-teal-50/40 to-emerald-100/40 border-t border-emerald-100/80 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 relative z-10">
        ...
        {/* Pillars Grid - 2 columns on all screens */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          {pillars.map((item, index) => (
            <motion.div 
              ...
              whileHover={{ y: -4 }}
              className="bg-white/40 backdrop-blur-xl border border-white/80 p-6 rounded-3xl shadow-xl hover:shadow-2xl transition-all duration-300 flex flex-col justify-between group relative overflow-hidden"
            >
              ...
              <div>
                <div className="flex items-center justify-between mb-4">
                  <div className="p-2.5 bg-white/70 border border-white/90 rounded-2xl shadow-sm backdrop-blur-md">
                    {item.icon}
                  </div>
                  <span className="text-[10px] font-extrabold text-emerald-900 uppercase tracking-wider bg-white/60 border border-white/80 backdrop-blur-md px-3 py-1 rounded-full shadow-sm">
                    {item.tag}
                  </span>
                </div>

                <h3 className="text-lg font-extrabold text-gray-900 leading-snug group-hover:text-emerald-950 transition-colors">
                  {item.title}
                </h3>
                <p className="text-[10px] font-bold text-emerald-800 mt-1">
                  {item.subtitle}
                </p>
                <p className="mt-3 text-gray-700 font-medium leading-relaxed text-sm">
                  {item.description}
                </p>
              </div>

              <div className="mt-6 pt-4 border-t border-white/60 flex items-center gap-2 text-[10px] font-bold text-emerald-900">
